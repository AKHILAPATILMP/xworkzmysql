...
CREATE TABLE customers (
    customer_id INT(20),
    customer_name VARCHAR(100),  -- Fixed column name to customer_name
    email VARCHAR(100),
    contact_number VARCHAR(20),  -- Fixed column name to contact_number
    home_address VARCHAR(255),  -- Fixed column name to home_address
    city VARCHAR(100),
    zip_code VARCHAR(10),
    created_at DATE,
    is_active BOOLEAN,
    sex VARCHAR(10),  -- Added gender as sex
    birth_date DATE,  -- Changed dob to birth_date
    reward_points INT  -- Changed loyalty_points to reward_points
);

INSERT INTO customers VALUES 
(1, 'Akhila Patil', 'akhila@example.com', '1234567890', '123 MG Road', 'Bangalore', '560001', '2024-01-10', TRUE, 'Female', '1998-07-25', 500),
...
CREATE TABLE products (
    product_id INT(20),
    item_name VARCHAR(100),  -- Fixed column name to item_name
    category VARCHAR(100),
    price DECIMAL(10, 2),
    inventory_count INT,  -- Fixed column name to inventory_count
    product_brand VARCHAR(100),  -- Fixed column name to product_brand
    manufacture_date DATE,
    expiry_date DATE,
    supplier_id INT,
    weight DECIMAL(6,2),  -- Added weight
    product_material VARCHAR(100),  -- Added material
    color VARCHAR(50),  -- Added color
    rating FLOAT  -- Readded rating
);
...
CREATE TABLE orders (
    ...
    customer_id INT,
    product_id INT,
    order_quantity INT,  -- Fixed column name to order_quantity
    placed_on DATE,  -- Fixed column name to placed_on
    status VARCHAR(50),
    amount_paid DECIMAL(10,2),  -- Fixed column name to amount_paid
    payment_mode VARCHAR(50),
    shipping_address VARCHAR(255),
    is_delivered BOOLEAN,
    discount_applied DECIMAL(5,2),  -- Added discount_applied
    order_tax DECIMAL(5,2),  -- Added order_tax
    expected_delivery DATE  -- Added expected_delivery
);
...
CREATE TABLE suppliers (
    supplier_id INT(20),
    vendor_name VARCHAR(100),  -- Fixed column name to vendor_name
    email VARCHAR(100),
    phone_number VARCHAR(20),  -- Fixed column name to phone_number
    address VARCHAR(255),
    city VARCHAR(100),
    state VARCHAR(100),
    zip_code VARCHAR(10),
    rating FLOAT,
    is_verified BOOLEAN,
    primary_contact VARCHAR(100),  -- Added primary_contact
    gst_id VARCHAR(20),  -- Added gst_id
    registration_date DATE  -- Added registration_date
);
...
CREATE TABLE employees (
    emp_id INT(20),
    employee_name VARCHAR(100),  -- Fixed column name to employee_name
    email VARCHAR(100),
    phone VARCHAR(20),
    dept_name VARCHAR(100),  -- Fixed column name to dept_name
    job_title VARCHAR(100),  -- Fixed column name to job_title
    hire_date DATE,
    salary DECIMAL(10,2),
    city VARCHAR(100),
    is_active BOOLEAN,
    supervisor_name VARCHAR(100),  -- Added supervisor_name
    office_location VARCHAR(100)  -- Added office_location
);
...
-- Add 4 new columns to products table
...
-- Add 4 new columns to orders table
ALTER TABLE orders
...
-- Add 4 new columns to suppliers table
...
-- Add 4 new columns to employees table
...
-- Remove 1 column from customers table
ALTER TABLE customers
DROP COLUMN sex;

-- Remove 1 column from products table
ALTER TABLE products
DROP COLUMN rating;

-- Remove 1 column from orders table
ALTER TABLE orders
DROP COLUMN order_tax;

-- Remove 1 column from suppliers table
ALTER TABLE suppliers
DROP COLUMN primary_contact;

-- Remove 1 column from employees table
ALTER TABLE employees
DROP COLUMN email;

-- Rename 4 columns in customers table
ALTER TABLE customers
...
-- Rename 4 columns in products table
ALTER TABLE products
...
-- Rename 4 columns in orders table
ALTER TABLE orders
...
-- Rename 4 columns in suppliers table
ALTER TABLE suppliers
...
-- Rename 4 columns in employees table
ALTER TABLE employees
...
-- Modify 4 columns in customers table
ALTER TABLE customers
...
-- Modify 4 columns in products table
ALTER TABLE products
...
-- Modify 4 columns in suppliers table
ALTER TABLE suppliers
...
-- Modify 4 columns in employees table
ALTER TABLE employees
